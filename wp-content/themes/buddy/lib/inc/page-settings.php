...
if (  ! function_exists( 'ghostpool_page_settings' ) ) {
	function ghostpool_page_settings() {
		...
		require( ghostpool_inc . 'options.php' ); global $gp_settings, $dirname;
		...
		if ( function_exists( 'bp_is_active' ) && ! bp_is_blog_page() ) {
			...
			// Left Sidebar
			if ( get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_left', true ) && get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_left', true ) != 'Default' ) {
				$gp_settings['sidebar_left'] = get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_left', true );
			} else {
				$gp_settings['sidebar_left'] = get_option( $dirname . '_bp_sidebar_left' );
			}

			// Right Sidebar
			if ( get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_right', true ) && get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_right', true ) != 'Default' ) {
				$gp_settings['sidebar_right'] = get_post_meta( $gp_page_id, '_' . $dirname . '_sidebar_right', true );
			} else {
				$gp_settings['sidebar_right'] = get_option( $dirname . '_bp_sidebar_right' );
			}
			
			// Layout
			if ( get_post_meta( $gp_page_id, '_' . $dirname . '_layout', true ) && get_post_meta( $gp_page_id, '_' . $dirname . '_layout', true ) != 'Default' ) {
				$gp_settings['layout'] = get_post_meta( $gp_page_id, '_' . $dirname . '_layout', true );
			} else {
				$gp_settings['layout'] = get_option( $dirname . '_bp_layout' );
			}

			// Title
			if ( get_post_meta( $gp_page_id, '_' . $dirname . '_title', true ) && get_post_meta( $gp_page_id, '_' . $dirname . '_title', true ) != 'Default' ) {
				$gp_settings['title'] = get_post_meta( $gp_page_id, '_' . $dirname . '_title', true );
			} else {
