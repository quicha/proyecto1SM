...
if ( ! function_exists( 'ghostpool_activity' ) ) {
	function ghostpool_activity( $atts, $content = null ) {
		...
		<?php if ( function_exists( 'bp_is_active' ) && bp_is_active( 'activity' ) ) { ?>
			...
			<div id="buddypress">
				...
				<div class="bp-wrapper gp-activity-stream activity gp-blog-wrapper<?php if ( $allow_comments == 'true' ) { ?> gp-allow-comments<?php } ?>">
					...
					<?php if ( bp_has_activities( bp_ajax_querystring( 'activity' ) . "&scope=$scope&per_page=$per_page&display_comments=$comments&sort=$order&max=$max&user_id=$user_id&count_total=count_query&page_arg=actsc" ) ) : ?>
						...
						<?php if ( bp_activity_has_more_items() && $pagination == 'true' ) : ?>
							...
							<?php if ( function_exists( 'bp_activity_load_more_link' ) ) { ?>
								...
								<li class="load-more">
									<a href="<?php bp_activity_load_more_link(); ?>"><?php esc_html_e( 'Load More', 'buddy' ); ?></a>
								</li>
					...
					<?php else : ?>
						...
						<div id="message" class="info">
							<p><?php esc_html_e( 'Sorry, there was no activity found. Please try a different filter.', 'buddy' ); ?></p>
						</div>
...
add_shortcode( 'activity', 'ghostpool_activity' );

// Change number of items per page in activity loop
if ( ! function_exists( 'ghostpool_activity_loop' ) ) {
	function ghostpool_activity_loop( $query_string, $object ) {
...
add_action( 'bp_ajax_querystring', 'ghostpool_activity_loop', 20, 2 );
