 ...
 */
 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
		 ...
		 * @var object
		 */
		public $theme_options_framework = 'buddy'; //supports radium framework and option tree
	    ...
	    public function __construct() {
			...
			//add_action( 'widgets_init', array( $this, 'gp_custom_sidebars' ) ); // MOD
		...
		// BEGIN MOD
		/*public function gp_custom_sidebars() {	
			if ( get_option( 'gp_theme_activated' ) !== '1' ) {
				...
				if( $sidebars !== FALSE ) {
					...
					add_option( 'cs_sidebars', $sidebars);
				}
				update_option( 'gp_theme_activated', '1' );
			}
    	...
    	public function intro_html() {
			...
			<p><?php esc_html_e( 'Importing demo data is the easiest way to setup your theme. It will allow you to quickly edit everything seen in the theme demo instead of creating everything from scratch. When you import the demo data the following things will happen:', 'buddy' ); ?></p>

			<ul class="ul-square">
				<li><?php esc_html_e( 'No existing data will be deleted or modified.', 'buddy' ); ?></li>
				<li><?php esc_html_e( 'No WordPress settings will be modified.', 'buddy' ); ?></li>
				<li><?php esc_html_e( 'Posts, pages, categories, media, widgets and menus will get imported.', 'buddy' ); ?></li>
			</ul>
		  
			<div class="update-nag">
				<p><strong><?php wp_kses( _e( '*** PLEASE READ BEFORE IMPORTING THE DEMO CONTENT ***<ol><li>You will see grey placeholder images instead of the images seen on the theme demo website due to copyright laws.</li><li>Please active BuddyPress and bbPress before importing the demo content. BuddyPress Activity, Groups and Members seen in the demo will NOT be imported. You can import dummy BuddyPress data using the <a href="https://wordpress.org/plugins/bp-default-data/" target="_blank">BuddyPress Default Data</a> plugin.</li></ol>', 'buddy' ), array( 'a' => array( 'href' => array(), 'target' => array() ) ) ); ?></strong></p>
			</div>
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_html__('Import Again', 'buddy');

			} else {

				$button_text = esc_html__('Import Demo Data', 'buddy');
			...
			<div class="wrap">
				...
				<h2><?php esc_html_e( 'Import Demo Data' , 'buddy' ); ?></h2>
				...
				<div class="radium-importer-wrap" data-demo-id="1" data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
					...
					<form method="post" class="js-one-click-import-form">
						...
						<input name="reset" class="panel-save button-primary" type="submit" value="<?php echo esc_attr( $button_text ); ?>" />
						<input type="hidden" name="action" value="demo-data" />      

						<script>
							jQuery( function ( $ ) {
								$( '.js-one-click-import-form' ).on( 'submit', function () {
									$( this ).append( '<br/><p><span class="spinner" style="visibility: visible; display: block; float: left; margin: 0 5px 0 0;"></span> <strong><?php esc_html_e( "Importing now, this can take a few minutes", "buddy" ); ?></strong></p>' );
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if( $this->theme_options_framework == 'buddy') {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'buddy' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'buddy' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'buddy' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'buddy' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'buddy' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'buddy' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'buddy' );
	    				}
	    			...
	    			//$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'buddy' ); // show "No Title" if widget instance is untitled
