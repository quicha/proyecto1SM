...
if ( ! class_exists( 'Radium_Theme_Demo_Data_Importer' ) ) {
	...
	class Radium_Theme_Demo_Data_Importer extends Radium_Theme_Importer {
		 ...
		 * Set framewok
		 *
		 * options that can be used are 'default', 'buddy' or 'optiontree'
		 *
		 * @since 0.0.3
		 *
		 * @var string
		 */
		public $theme_options_framework = 'buddy';
		...
		public function set_demo_menus(){
			...
			if ( $gp_menus ) {
				foreach( $gp_menus as $gp_menu ) { // assign menus to theme locations
					if ( $gp_menu->name == 'Header' ) {
						$gp_locations['main-nav'] = $gp_menu->term_id;				
					}
				}
			}
