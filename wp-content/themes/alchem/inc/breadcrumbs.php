...
* @return void
*/
function alchem_breadcrumb_trail( $args = array() ) {
if ( function_exists( 'is_bbpress' ) && is_bbpress() )
$breadcrumb = new alchem_bbPress_Breadcrumb_Trail( $args );
else
$breadcrumb = new alchem_Breadcrumb_Trail( $args );
...
$labels = array(
'browse' => __( 'Browse:', 'alchem' ),
'home' => __( 'Home', 'alchem' ),
'error_404' => __( '404 Not Found', 'alchem' ),
'archives' => __( 'Archives', 'alchem' ),
/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
'search' => __( 'Search results for &#8220;%s&#8221;', 'alchem' ),
/* Translators: %s is the page number. */
'paged' => __( 'Page %s', 'alchem' ),
/* Translators: Minute archive title. %s is the minute time format. */
'archive_minute' => __( 'Minute %s', 'alchem' ),
/* Translators: Weekly archive title. %s is the week date format. */
'archive_week' => __( 'Week %s', 'alchem' ),
...
public function do_singular_items() {
 ...
 $show_categories        = alchem_option('breadcrumb_show_categories','yes');
 $show_post_type_archive = alchem_option('breadcrumb_show_post_type_archive','yes');
...
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'alchem' ) ) );
}
/**
...
/* Add the minute item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'alchem' ) ) );
}
/**
...
/* Add the hour item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'alchem' ) ) );
}
/**
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'alchem' ) ) );
$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'alchem' ) ) );
$day = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'alchem' ) ) );
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'alchem' ) ) );
$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'alchem' ) ) );
/* Add the year item. */
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( $year ) . '">' . $year . '</a>';
/* Add the week item. */
if ( is_paged() )
...
if ( '%year%' == $tag )
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'alchem' ) ) ) . '</a>';
/* If using the %monthnum% tag, add a link to the monthly archive. */
elseif ( '%monthnum%' == $tag )
$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'alchem' ) ) ) . '</a>';
/* If using the %day% tag, add a link to the daily archive. */
elseif ( '%day%' == $tag )
$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'alchem' ) ) ) . '</a>';
...
* @access public
*/
class alchem_bbPress_Breadcrumb_Trail extends alchem_Breadcrumb_Trail {
/**
